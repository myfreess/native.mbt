...
MOONBIT_EXPORT int32_t moonbit_file_read(FILE file, moonbit_bytes_t buf, int32_t offset, int32_t capacity) {
    DWORD bytes_read;
    BOOL ret = ReadFile(file, (buf + offset), capacity - offset, &bytes_read, NULL);
    if (!ret) {
        return -1;
    }
    return bytes_read;
...
MOONBIT_EXPORT int32_t moonbit_file_write(FILE file, moonbit_bytes_t buf, int32_t offset, int32_t capacity) {
    DWORD bytes_written;
    BOOL ret = WriteFile(file, (buf + offset), capacity - offset, &bytes_written, NULL);
    if (!ret) {
        return -1;
    }
...
MOONBIT_EXPORT int32_t moonbit_file_read(FILE file, uint8_t* buf, int32_t offset, int32_t capacity) {
    // assert offset > 0 && len(buf) == capacity
    int32_t bytes_read = read(file, (buf + offset), capacity - offset);
    return bytes_read;
}

MOONBIT_EXPORT int32_t moonbit_file_write(FILE file, uint8_t* buf, int32_t offset, int32_t capacity) {
    // assert offset > 0 && len(buf) == capacity
    int32_t bytes_written = write(file, (buf + offset), capacity - offset);
    return bytes_written;
}
